...
| **player distance** | a hard cutoff on the tested board: all 94 no-pure-saddle states have the players within 2 cells (Manhattan) of each other, none at distance ≥ 3 -- proximity to the opponent, not to the goal; full per-distance table (states, no-pure-saddle, forced vs. degenerate) in [showcase.md](showcase.md) | [showcase.md](showcase.md) |
