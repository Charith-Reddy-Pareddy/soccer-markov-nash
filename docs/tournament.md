...
- **hand-built** -- the scripted chase-and-score policy
...
|---|---|---|---|---|
| **minimax** | +0.67 | +0.71 | **+0.15** | +0.52 |
| greedy/rand | **+0.92** | +1.03 | **−0.59** | +1.51 |
...
`+0.57 → +0.17` against its challenger, greedy/rand `+0.59 → −0.29`.)
...
3. **greedy/self and hand-built do neither.** The maximin-security policy is
   unexploitable but toothless (`+0.05` vs. a random opponent -- it never
   presses an advantage). The scripted policy is both beatable by a challenger
   (`−1.03`) and no better than a coin flip otherwise.
