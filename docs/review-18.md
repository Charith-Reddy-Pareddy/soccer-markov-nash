...
| 2 | Central question -> "which geometries force mixing, can they be characterized analytically"; two contributions (algorithmic + structural) | pending -- report reframe |
...
| 7 | "25x" wording: keep LP-solve reduction and wall-clock speedup separate | pending -- wording pass |
| 8 | Explicit stage-game taxonomy (exact / numerically-indistinguishable / strict / degenerate / genuine mixed) | pending -- `classify_stage_game` docstring + report |
...
| 13 | Paper structure not centered on A10 Part 2 | pending -- report reorder |
| 14 | Title -> "When Does a Soccer Markov Game Need Mixed Strategies?" | pending |
| 15 | Novelty = the geometric characterization of unavoidable mixing | pending -- framing |
...
## This pass (points 1, 9, 10, 17, 18)

`soccer_nash/templates.py` + `scripts/templates.py` + `docs/templates.md`:
the 94 no-pure-saddle states reduce to **8 geometric templates**; the **four
with a 2x2 equilibrium support are all genuine matching pennies** (best replies
cross both ways), covering **68 of 94** states. The remaining 26 are borderline
(a near-pure saddle) or need a 3x3 mix. The precise analytic condition for an
unavoidable mixed stage game:
...
- **2 / 7 / 8 / 13 / 14 / 15** -- one editing pass on `docs/report.md`:
  reframe around the structural question, add the stage-game taxonomy, split the
  "25x" wording into "LP-solve reduction" vs "wall-clock", reorder so A10 Part 2
  is a verification appendix, retitle.
- **4 / 5 / 6 / 12** -- one measurement pass: re-run the sweeps recording seeds
  ...
  medians and add LP-calls-per-state. The board sweep alone is ~10 min.
