...
| 4 | Board sweep -> phase diagram: mixed / reachable vs width x height x goal width; compare board width vs goal width | `scripts/phase_diagram.py` (all boards `w<=11, h<=9, w*h<=45`, every goal width), `experiments/phase_diagram.csv`, `figures/png/phase_diagram.png`; `--analyze` does the variance decomposition -- goal width (1 vs >=2) perfectly predicts *whether* mixing occurs, board area explains the residual *fraction* |
